...
public class Level {
	...
	numSquaresToFill--;
		...
		// every tile made to placedThisRound
	...
	private void water(int col, int row, Map map, int fullness) {
		...
		Water f = new Water(col, row, tileSize, tileset.getImage(size), this, fullness);
		map.addTile(col, row, f);
		waters.add(f);
		...
		if (row + 1 < map.getTiles()[0].length && !(map.getTiles()[col][row + 1] instanceof Water)
			...
			if (row + 2 < map.getTiles()[0].length
					&& ((map.getTiles()[col][row + 2] instanceof Water) || map.getTiles()[col][row + 2].isSolid()))
